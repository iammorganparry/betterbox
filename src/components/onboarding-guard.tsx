...
export function OnboardingGuard({ children }: OnboardingGuardProps) {
	...
	} = api.subscription.getOnboardingStatus.useQuery(undefined, {
		...
		if (isSignedIn && user) {
			...
			if (onboardingStatus) {
				...
				if (requiresOnboarding && !isOnboardingRoute && !isApiRoute) {
					console.log(
						`User ${user.id} requires onboarding, redirecting from ${pathname}`,
					);
					router.push("/onboarding");
					return;
				}
...
export function useOnboardingStatus() {
	...
	const query = api.subscription.getOnboardingStatus.useQuery(undefined, {
