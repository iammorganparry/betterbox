...
describe("subscriptionRouter.completeOnboarding", () => {
	...
	describe("Stripe customer creation", () => {
		it("should create a new Stripe customer when user does not have one", async () => {
			...
			mockOnboardingService.getOnboardingStatus
				.mockResolvedValueOnce(mockOnboardingStatus)
				.mockResolvedValueOnce(mockVerificationStatus);
			mockOnboardingService.validateOnboardingIntegrity.mockResolvedValue(
				mockIntegrity,
			);
		...
		it("should handle Stripe customer creation failure", async () => {
			...
			mockUserService.findById.mockResolvedValue(mockUser);
			mockStripeService.createOrGetCustomer.mockRejectedValue(
			...
			await expect(caller.completeOnboarding(input)).rejects.toThrow(
		...
		it("should handle payment method attachment failure", async () => {
			...
			mockUserService.findById.mockResolvedValue(mockUser);
			mockStripeService.attachPaymentMethod.mockRejectedValue(
				new Error("Payment method error"),
			);
