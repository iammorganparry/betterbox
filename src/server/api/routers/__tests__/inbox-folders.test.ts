...
describe('inboxRouter - Folder Management', () => {
  ...
  describe('getFolders', () => {
    it('should return user folders with chat counts', async () => {
      ...
      mockChatFolderService.getFoldersWithChatCounts.mockResolvedValue(expectedFolders)
      ...
      // Assert
      expect(mockChatFolderService.getFoldersWithChatCounts).toHaveBeenCalledWith('user-123')
    ...
    it('should throw INTERNAL_SERVER_ERROR when database operation fails', async () => {
      // Arrange
      const ctx = createMockContext()
      mockChatFolderService.getFoldersWithChatCounts.mockRejectedValue(new Error('Database error'))

      const caller = inboxRouter.createCaller(ctx)

      // Act & Assert
      await expect(caller.getFolders()).rejects.toThrow('Failed to fetch folders')
    })
  ...
  describe('createFolder', () => {
    it('should create folder successfully', async () => {
      ...
      // Assert
      expect(mockChatFolderService.createFolder).toHaveBeenCalledWith('user-123', input)
    ...
    it('should handle duplicate folder name error', async () => {
      ...
      // Act & Assert
      await expect(caller.createFolder(input)).rejects.toThrow('Failed to create folder')
    })
  })
  ...
  describe('updateFolder', () => {
    ...
    it('should validate folder name when provided', async () => {
      ...
      // Act & Assert
      await expect(caller.updateFolder({
        folderId: 'folder-123',
        name: '', // Empty name
      })).rejects.toThrow()
    ...
    it('should handle folder not found error', async () => {
      ...
      // Act & Assert
      await expect(caller.updateFolder(input)).rejects.toThrow('Failed to update folder')
    })
  })
  ...
  describe('deleteFolder', () => {
    it('should delete folder successfully', async () => {
      ...
      // Assert
      expect(mockChatFolderService.deleteFolder).toHaveBeenCalledWith('folder-123', 'user-123')
    ...
    it('should handle folder not found error', async () => {
      ...
      // Act & Assert
      await expect(caller.deleteFolder(input)).rejects.toThrow('Failed to delete folder')
    })
  })
})
...
describe('inboxRouter - Chat Folder Assignments', () => {
  ...
  describe('assignChatToFolder', () => {
    it('should assign chat to folder successfully', async () => {
      ...
      mockChatFolderService.assignChatToFolder.mockResolvedValue(expectedAssignment)
      ...
      // Assert
      expect(mockUnipileChatService.getChatWithDetails).toHaveBeenCalledWith('chat-123')
      expect(mockChatFolderService.getFolderById).toHaveBeenCalledWith('folder-123', 'user-123')
    ...
    it('should handle duplicate assignment error', async () => {
      ...
      mockUnipileChatService.getChatWithDetails.mockResolvedValue(mockChat)
      mockChatFolderService.getFolderById.mockResolvedValue(mockFolder)
      mockChatFolderService.assignChatToFolder.mockRejectedValue(duplicateError)

      const caller = inboxRouter.createCaller(ctx)

      // Act & Assert
      await expect(caller.assignChatToFolder(input)).rejects.toThrow('Failed to assign chat to folder')
    })
  })
  ...
  describe('removeChatFromFolder', () => {
    it('should remove chat from folder successfully', async () => {
      ...
      mockChatFolderService.removeChatFromFolder.mockResolvedValue(updatedAssignment)
      ...
      // Assert
      expect(mockChatFolderService.removeChatFromFolder).toHaveBeenCalledWith('chat-123', 'folder-123')
    ...
    it('should handle assignment not found error', async () => {
      ...
      mockChatFolderService.removeChatFromFolder.mockRejectedValue(notFoundError)

      const caller = inboxRouter.createCaller(ctx)

      // Act & Assert
      await expect(caller.removeChatFromFolder(input)).rejects.toThrow('Failed to remove chat from folder')
    })
  })
  ...
  describe('getChatsInFolder', () => {
    it('should return chats in folder successfully', async () => {
      ...
      mockChatFolderService.getFolderById.mockResolvedValue(mockFolder)
      mockChatFolderService.getChatsInFolder.mockResolvedValue(expectedAssignments)
      ...
      // Assert
      expect(mockChatFolderService.getFolderById).toHaveBeenCalledWith('folder-123', 'user-123')
      expect(mockChatFolderService.getChatsInFolder).toHaveBeenCalledWith('folder-123')
  ...
  describe('getChatFolders', () => {
    it('should return folders for a chat successfully', async () => {
      ...
      mockUnipileChatService.getChatWithDetails.mockResolvedValue(mockChat)
      mockChatFolderService.getChatFolders.mockResolvedValue(expectedAssignments)
      ...
      // Assert
      expect(mockUnipileChatService.getChatWithDetails).toHaveBeenCalledWith('chat-123')
      expect(mockChatFolderService.getChatFolders).toHaveBeenCalledWith('chat-123')
      expect(result).toEqual(expectedAssignments)
    })
