...
describe('inboxRouter.sendMessage', () => {
  ...
  it('should send a message successfully', async () => {
    // Arrange
    mockContext.services.unipileChatService.getChatWithDetails.mockResolvedValue(mockChatData)
    mockUnipileService.sendMessage.mockResolvedValue(mockUnipileResponse.sendMessage)
    mockContext.services.unipileMessageService.upsertMessage.mockResolvedValue(mockMessageData)
    ...
    expect(mockContext.services.unipileChatService.getChatWithDetails).toHaveBeenCalledWith('chat-123')
    ...
    expect(mockContext.services.unipileMessageService.upsertMessage).toHaveBeenCalled()
    expect(mockContext.services.unipileChatService.updateLastMessageAt).toHaveBeenCalledWith(
      'chat-123',
      expect.any(Date)
    )
  })
  ...
  it('should throw NOT_FOUND error when chat does not exist', async () => {
    // Arrange
    mockContext.services.unipileChatService.getChatWithDetails.mockResolvedValue(null)
  ...
  it('should throw FORBIDDEN error when user does not own the chat', async () => {
    ...
    mockContext.services.unipileChatService.getChatWithDetails.mockResolvedValue(otherUserChat)
  ...
  it('should throw FORBIDDEN error when chat is read-only', async () => {
    ...
    mockContext.services.unipileChatService.getChatWithDetails.mockResolvedValue(readOnlyChat)
  ...
  it('should handle case when Unipile does not return message data', async () => {
    ...
    // Assert
    expect(result.savedMessage).toBeNull()
    expect(mockContext.services.unipileMessageService.upsertMessage).not.toHaveBeenCalled()
    expect(mockContext.services.unipileChatService.updateLastMessageAt).toHaveBeenCalled()
  })
