...
describe("ChatFolderService - Folder Management", () => {
	...
	describe("getFoldersByUser", () => {
		it("should return all folders for a user with default options", async () => {
			...
			mockPrismaClient.chatFolder.findMany = vi
				.fn()
	...
	describe("getFolderById", () => {
		it("should return folder when it exists and belongs to user", async () => {
			...
			mockPrismaClient.chatFolder.findFirst = vi
				.fn()
	...
	describe("updateFolder", () => {
		it("should update folder successfully", async () => {
			...
			mockPrismaClient.chatFolder.update = vi
				.fn()
...
describe("ChatFolderService - Folder Assignments", () => {
	...
	describe("assignChatToFolder", () => {
		it("should create new assignment when none exists", async () => {
			...
			mockPrismaClient.chatFolderAssignment.findFirst = vi
				.fn()
				.mockResolvedValue(null);
			mockPrismaClient.chatFolderAssignment.create = vi
				.fn()
				.mockResolvedValue(expectedAssignment);
			...
			// Assert
			expect(
				mockPrismaClient.chatFolderAssignment.findFirst,
		...
		it("should update existing assignment (restore if soft-deleted)", async () => {
			...
			mockPrismaClient.chatFolderAssignment.update = vi
				.fn()
	...
	describe("removeChatFromFolder", () => {
		it("should remove chat from folder successfully", async () => {
			...
			// Act
			const result = await chatFolderService.removeChatFromFolder(
				chatId,
				folderId,
			);
	...
	describe("getChatFolders", () => {
		it("should return all folders for a chat", async () => {
			...
			mockPrismaClient.chatFolderAssignment.findMany = vi
				.fn()
				.mockResolvedValue(expectedAssignments);
			...
			// Assert
			expect(
				mockPrismaClient.chatFolderAssignment.findMany,
			).toHaveBeenCalledWith({
				where: {
					chat_id: chatId,
					is_deleted: false,
	...
	describe("getFoldersWithChatCounts", () => {
		it("should return folders with chat counts", async () => {
			...
			mockPrismaClient.chatFolder.findMany = vi
				.fn()
				.mockResolvedValue(mockFoldersWithCounts);
	...
	describe("bulkAssignChatsToFolder", () => {
		it("should assign multiple chats to folder successfully when no existing assignments", async () => {
			// Arrange
			...
			mockPrismaClient.chatFolderAssignment.updateMany = vi
				.fn()
				.mockResolvedValue({ count: 0 });
			mockPrismaClient.chatFolderAssignment.findMany = vi
				.fn()
				.mockResolvedValueOnce([]) // First call returns no existing assignments
				.mockResolvedValueOnce(expectedAssignments); // Second call returns final result
			mockPrismaClient.chatFolderAssignment.createMany = vi
				.fn()
			...
			// Assert
			expect(
				mockPrismaClient.chatFolderAssignment.updateMany,
			).toHaveBeenCalledWith({
				where: {
					chat_id: { in: chatIds },
					folder_id: folderId,
			...
			expect(
				mockPrismaClient.chatFolderAssignment.createMany,
			).toHaveBeenCalledWith({
				data: [
					{
						chat_id: "chat-1",
		...
		it("should handle empty chat list", async () => {
			...
			mockPrismaClient.chatFolderAssignment.findMany = vi
				.fn()
				.mockResolvedValueOnce([]) // First call for existing assignments
				.mockResolvedValueOnce([]); // Second call for final result
			...
			// Should not call createMany for empty list
			expect(
				mockPrismaClient.chatFolderAssignment.createMany,
		...
		it("should skip duplicates when assigning chats", async () => {
			...
			const folderId = "folder-123";
			const assignedById = "user-123";
			const existingAssignment = { chat_id: "chat-1" };
			const newAssignment = {
				id: "assignment-chat-2",
				chat_id: "chat-2",
				folder_id: folderId,
				assigned_by_id: assignedById,
				is_deleted: false,
			...
			// Mock existing assignment for chat-1
			mockPrismaClient.chatFolderAssignment.updateMany = vi
				.fn()
				.mockResolvedValue({ count: 0 });
			mockPrismaClient.chatFolderAssignment.findMany = vi
				.fn()
				.mockResolvedValueOnce([existingAssignment]) // First call returns existing assignment
				.mockResolvedValueOnce([newAssignment]); // Second call returns final result
			mockPrismaClient.chatFolderAssignment.createMany = vi
				.fn()
			...
			// Assert - should only create assignment for chat-2 (chat-1 already exists)
			expect(
				mockPrismaClient.chatFolderAssignment.createMany,
			).toHaveBeenCalledWith({
				data: [
